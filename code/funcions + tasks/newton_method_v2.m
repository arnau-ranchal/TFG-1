...
function rho = newton_method_v2(x, SNR, Q, pi_, G, max_iteracions, threshold, R)
...
%         rho = hermite_interp(punt1, punt2, dp1, dp2); % Paràmetres: [E0(1) - R, E0'(0)-R, E0'(1) - R] 
        ...
        for iteration = 1:max_iteracions
            % Derivades
            [f, fp] = F0_F0p(Q, pi_, G, rho); % Això s'ha de canviar per la fixed un cop estigui fixed de veritat
